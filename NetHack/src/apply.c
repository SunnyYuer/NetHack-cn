...
struct obj *obj;
{
    ...
    if (!freehand()) {
        ...
        case 1:
            if (!ublindf) {
                old = u.ucreamed;
                u.ucreamed += rn1(10, 3);
                pline("哟! 你的%s因它变得%s粘性!", body_part(FACE),
                      (old ? "更加" : "有"));
    ...
    if (Glib) {
        Glib = 0;
        You("擦你的%s.", makeplural(body_part(HAND)));
        if (is_wet_towel(obj))
            dry_a_towel(obj, -1, drying_feedback);
        return 1;
    } else if (u.ucreamed) {
...
struct obj *obj;
{
    ...
    if (!can_blow(&youmonst)) {
        ...
        You(whistle_str,
            Hallucination ? "标准的" : Underwater ? "奇怪的, 尖锐的"
                                                  : "奇怪的");
...
#define SEENMON (MONSEEN_NORMAL | MONSEEN_SEEINVIS | MONSEEN_INFRAVIS)
    ...
    if (mtmp->msleeping) {
        ...
        if (vis) {
            ...
            pline("%s 欣赏%s自己在你的%s中.", Monnam(mtmp), mhim(mtmp),
                  mirror);
            pline("%s 拿走了它!", upstart(strcpy(buf, mhe(mtmp))));
        } else
...
struct obj *obj;
{
    ...
    if (obj->spe > 0) {
        ...
        if (otmp != &zeroobj) {
            ...
            incr_itimeout(&Glib, rnd(15));
            You("用油脂包裹了你的%s.", makeplural(body_part(FINGER)));
        }
    } else {
        if (obj->known)
...
struct obj *tstone;
{
    ...
    choices = (tstone->otyp == TOUCHSTONE && tstone->dknown
               && objects[TOUCHSTONE].oc_name_known)
                  ? coins_gems
                  : allowall;
    Sprintf(stonebuf, "在石头上擦拭%s", plur(tstone->quan));  //rub on the stone
...
struct obj *otmp;
{
    ...
    if (u.usteed && P_SKILL(P_RIDING) < P_BASIC) {
        ...
        if (Fumbling || otmp->cursed)
            ...
            chance = (rnl(10) > 5);
        You("没有非常熟练从%s 够到.", mon_nam(u.usteed));
