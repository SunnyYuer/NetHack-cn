    ...
    if (!Upolyd) {
        ...
        if (wizard) {
            if (u.ulevel < 30) {
                ...
                Sprintf(eos(buf), ", 还需%ld%s%s来升到等级%d",
                        (nxtlvl - u.uexp), (u.uexp > 0) ? "" : "",
                        !final ? "" : "", (ulvl + 1));
            }
        }
        you_have(buf, "");
...
int final;
{
    ...
    if (hp == hpmax && hpmax > 1)
        Sprintf(buf, "有%d HP", hpmax);
    else
        Sprintf(buf, "有%d HP, 上限%d HP", hp, hpmax);
    you_have(buf, "");
    ...
    if (pwmax == 0 || (pw == pwmax && pwmax == 2)) /* both: "all 2" is silly */
        ...
        Sprintf(buf, "有%d %s, 上限%d %s", pw, Power, pwmax, Power);
    you_have(buf, "");
