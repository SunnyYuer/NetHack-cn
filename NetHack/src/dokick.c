...
dokick()
{
    ...
    if (!IS_DOOR(maploc->typ)) {
        ...
        if (IS_SINK(maploc->typ)) {
            ...
            if (rn2(5)) {
                ...
                /* can't resist... */
                pline("%s回来了!", (Blind ? Something : "洗碗工"));
                ...
                if (Blind && Deaf)
                    Sprintf(buf, "%s上", body_part(FACE));
                else
                    buf[0] = '\0';
                pline("%s%s%s.", !Deaf ? "泼溅声! " : "",
                      !Blind
                          ? "泥泞的垃圾从下水道出来"
                          : !Deaf
                              ? "你听见泼溅的声音"  /* Deaf-aware */
 ...
 ouch:
            ...
            losehp(Maybe_Half_Phys(dmg), kickstr(buf, kickobjnam), DIE_OF);
...
xchar dlev;          /* if !0 send to dlev near player */
{
    ...
    if (dct && cansee(x, y)) { /* at least one object fell */
        const char *what = (dct == 1L ? "物品掉" : "物品掉");

        if (missile)
            pline("受其影响, 另外的%s%s落了.",
                  dct == oct ? "" : dct == 1L ? "一个" : "", what);
        else if (oct == dct)
            pline("%s邻近的%s%s去了.", dct == 1L ? "" : "所有", what,
                  gate_str);
        else
            pline("%s邻近的%s%s去了.",
                  dct == 1L ? "一个" : "一些",
                  dct == 1L ? "物品掉" : what, gate_str);
    }
...
long num;
{
    ...
    if (num) { /* means: other objects are impacted */
        ...
        Sprintf(xbuf, " %s%s东西%s", otense(otmp, "打中"),
                (num == 1L) ? "另一个" : "其他的", (num > 1L) ? "" : "");
        if (nodrop)
            Sprintf(eos(xbuf), ".");
        else
            Sprintf(eos(xbuf), "然后%s%s去了.", otense(otmp, "掉"), gate_str);
        pline("%s%s", obuf, xbuf);
    } else if (!nodrop)
        pline("%s%s%s去了.", obuf, otense(otmp, "掉"), gate_str);
}
...
xchar x, y;
{
    ...
    if (((ttmp = t_at(x, y)) != 0 && ttmp->tseen)
        && is_hole(ttmp->ttyp)) {
        gate_str = (ttmp->ttyp == TRAPDOOR) ? "过陷阱门"
                                            : "过洞";
        return MIGR_RANDOM;
    }
    return MIGR_NOWHERE;
