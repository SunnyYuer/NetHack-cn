...
boolean verbosely; /* usually True; False if caller has given drop message */
{
    ...
    if (IS_ALTAR(levl[u.ux][u.uy].typ))
        ...
        pline("%s%s%s.", Doname2(obj), otense(obj, "打中了"),
              surface(u.ux, u.uy));
...
int x, y;
{
    ...
    if (!Passes_walls || !(may_pass = may_passwall(x, y))) {
        ...
        if (IS_ROCK(levl[x][y].typ) || closed_door(x, y) || odoor_diag) {
            ...
            if (IS_TREE(levl[x][y].typ))
                ...
                s = "碰到门";
            dmg = rnd(2 + *range);
            losehp(Maybe_Half_Phys(dmg), s, DIE_OF);
            wake_nearto(x,y, 10);
            return FALSE;
        }
        ...
        if ((obj = sobj_at(BOULDER, x, y)) != 0) {
            ...
            dmg = rnd(2 + *range);
            losehp(Maybe_Half_Phys(dmg), "撞上巨石", DIE_OF);
            wake_nearto(x,y, 10);
            return FALSE;
        }
        if (!may_pass) {
    ...
    if ((mon = m_at(x, y)) != 0
        ...
        if (!strcmp(mnam, "它")) {
            mnam = !strcmp(pronoun, "它") ? "什么东西" : "某人";
        }
        if (!glyph_is_monster(glyph) && !glyph_is_invisible(glyph))
            You("通过撞到%s发现了%s.", pronoun, mnam);
        else
            You("撞到%s.", mnam);
        wakeup(mon, FALSE);
        if (!canspotmon(mon))
    ...
    if (is_pool(x, y) && !u.uinwater) {
        if ((Is_waterlevel(&u.uz) && levl[x][y].typ == WATER)
            ...
            Norep("你越过%s.", is_moat(x, y) ? "护城河" : "水池");
       }
...
boolean twoweap; /* used to restore twoweapon mode if wielded weapon returns */
{
    ...
    if (!thrownobj) {
        ...
        if (tethered_weapon) {
            tmp_at(DISP_END, 0);
            pline("%s回到了你的手中!", The(xname(thrownobj)));
