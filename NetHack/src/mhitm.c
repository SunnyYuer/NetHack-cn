...
struct attack *mattk;
{
    ...
    if (vis) {
        ...
        if ((compat = could_seduce(magr, mdef, mattk)) && !magr->mcan) {
            ...
            switch (mattk->aatyp) {
            case AT_BITE:
                Sprintf(buf, "%s咬了一口", magr_name);
                break;
            case AT_STNG:
                Sprintf(buf, "%s叮了一口", magr_name);
                break;
            case AT_BUTT:
                Sprintf(buf, "%s撞了一下", magr_name);
                break;
            case AT_TUCH:
                Sprintf(buf, "%s碰了一下", magr_name);
                break;
            case AT_TENT:
                Sprintf(buf, "%s触手吸食", s_suffix(magr_name));
                break;
            case AT_HUGS:
                if (magr != u.ustuck) {
                    Sprintf(buf, "%s挤压", magr_name);
                    break;
                }
                /*FALLTHRU*/
            default:
                Sprintf(buf, "%s打了一下", magr_name);
            }
            pline("%s %s.", buf, mon_nam_too(mdef, magr));
 ...
 physical:
    ...
    case AD_FIRE:
        ...
        if (resists_fire(mdef)) {
            if (vis && canseemon(mdef))
                pline_The("火似乎没有烧伤%s!", mon_nam(mdef));
 ...
 post_stone:
    ...
    case AD_SLEE:
        if (!cancelled && !mdef->msleeping
            ...
            if (vis && canspotmon(mdef)) {
                Strcpy(buf, Monnam(mdef));
                pline("%s被%s陷入了沉睡.", buf, mon_nam(magr));
            }
            mdef->mstrategy &= ~STRAT_WAITFORU;
            slept_monst(mdef);
        }
...
int mdead;
{
    ...
    if (rn2(3))
        ...
        case AD_FIRE:
            ...
            if (canseemon(magr))
                pline("%s突然非常热!", Monnam(magr));
            break;
        case AD_ELEC:
            if (resists_elec(magr)) {
                if (canseemon(magr)) {
                    pline("%s轻微的刺痛.", Monnam(magr));
