...
register struct monst *mtmp;
{
            ...
            if (youseeit) {
                ...
                if (numhelp > 0) {
                    if (numseen < 1)
                        ...
                        if (numseen == 1)
                            Sprintf(buf, "一只%s 出现了", genericwere);
                        else
                            Sprintf(buf, "%s出现了",
    ...
    if (u.uinvulnerable) {
        ...
        if (mtmp == u.ustuck)
            pline("%s 轻微地放松了其控制.", Monnam(mtmp));
        else if (!range2) {
            if (youseeit || sensemon(mtmp))
                pline("%s 开始攻击你, 但又撤回.",
                      Monnam(mtmp));
            else
                You_feel("%s 在附近移动.", something);
        }
        return 0;
    }
...
register struct attack *mattk;
{
    ...
    case AD_LEGS: {
        ...
        if (u.usteed || Levitation || Flying) {
            pline("%s 试图够到你的%s %s!", Monst_name, sidestr, leg);
            dmg = 0;
        } else if (mtmp->mcan) {
    ...
    if (rn2(3))
                ...
                if (mtmp->mcansee && haseyes(mtmp->data) && rn2(3)
                    ...
                    if (Blind)
                        ...
                        if (mon_reflects(mtmp,
                                         "你的凝视被 %s %s反射."))
                            return 1;
                        pline("%s 被你的凝视所冰冻!", Monnam(mtmp));
                        paralyze_monst(mtmp, tmp);
                        return 3;
                    }
                }
            } else { /* gelatinous cube */
                pline("%s 被你所冰冻了.", Monnam(mtmp));
                paralyze_monst(mtmp, tmp);
                return 3;
            }
            return 1;
        case AD_COLD: /* Brown mold or blue jelly */
            if (resists_cold(mtmp)) {
                ...
                pline("%s 轻微的寒冷.", Monnam(mtmp));
                golemeffects(mtmp, AD_COLD, tmp);
                tmp = 0;
                break;
            }
            pline("%s 突然非常热!", Monnam(mtmp));
            u.mh += tmp / 2;
            if (u.mhmax < u.mh)
                u.mhmax = u.mh;
        ...
        case AD_ELEC:
            if (resists_elec(mtmp)) {
                shieldeff(mtmp->mx, mtmp->my);
                pline("%s 轻微的刺痛.", Monnam(mtmp));
                golemeffects(mtmp, AD_ELEC, tmp);
                tmp = 0;
                break;
            }
            pline("%s 被你电击到!", Monnam(mtmp));
            break;
        default:
            tmp = 0;
            break;
