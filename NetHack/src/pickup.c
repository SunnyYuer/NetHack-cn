...
static const char
        moderateloadmsg[] = "你有些困难地拾起",
        nearloadmsg[] = "你很困难地拾起",
        overloadmsg[] = "你非常艰难地拾起";
...
int *menu_on_demand;
{
 ...
 ask_again:
        ...
        *one_at_a_time = *everything = FALSE;
        ...
        Sprintf(qbuf, "你想%s 哪种东西? [%s]", action,
                ilets);
        getlin(qbuf, inbuf);
        if (*inbuf == '\033')
            return FALSE;
...
boolean FDECL((*allow), (OBJ_P)); /* allow function */
{
    ...
    *pick_list = (menu_item *) 0;
    ...
    if (engulfer) {
        ...
        if (sorted && n > 1) {
            Sprintf(buf, "%s 生物",
                    is_animal(u.ustuck->data) ? "被吞食的" : "被吞没的");
    ...
    if (n > 0) {
        ...
        for (i = k = 0, mi = *pick_list; i < n; i++, mi++) {
            ...
            if (engulfer_minvent && curr->owornmask != 0L) {
                You_cant("拾取%s.", ysimple_name(curr));
                continue;
            }
...
int how;               /* type of query */
{
    ...
    *pick_list = (menu_item *) 0;
    ...
    if (do_unpaid) {
        ...
        any = zeroany;
        any.a_int = 'u';
        add_menu(win, NO_GLYPH, &any, invlet, 0, ATR_NONE, "未付款项",
                 MENU_UNSELECTED);
    }
...
boolean telekinesis;
{
    ...
    if (obj->otyp == LOADSTONE
        ...
        You("携带了太多的东西不能拾取%s%s.",
            (obj->quan == 1L) ? "另一个" : "更多的", simpleonames(obj));
        return -1;
    }
...
boolean telekinesis; /* not picking it up directly by hand */
{
    ...
    if (obj == uchain) { /* do not pick up attached chain */
        ...
        You_cant("拾取%s.", ysimple_name(obj));
        return 0;
        ...
        if (obj->blessed)
            ...
            pline_The("在你%s卷轴时%s%s了灰尘.",
                      telekinesis ? "举起" : "捡起", (obj->quan == 1L) ? "它" : "它们",
                      otense(obj, "变成"));
 ...
 lootmon:
    if (c != 'y' && mon_beside(u.ux, u.uy)) {
        ...
        if (!underfoot) {
            if (container_at(cc.x, cc.y, FALSE)) {
                if (mtmp) {
                    You_cant("搜刮%s挡的地方的任何%s东西.",
                             mon_nam(mtmp), prev_inquiry ? "别的 " : "");
                    return timepassed;
                } else {
                    You("必须在一旁才能搜刮.");
                }
...
boolean *prev_loot;
{
    ...
    if (mtmp && mtmp != u.usteed && (otmp = which_armor(mtmp, W_SADDLE))) {
        ...
        if ((c = yn_function(qbuf, ynqchars, 'n')) == 'y') {
            ...
            otmp = hold_another_object(otmp, "你扔掉了 %s!", doname(otmp),
                                       (const char *) 0);
            nhUse(otmp);
            timepassed = rnd(3);
            if (prev_loot)
...
register struct obj *obj;
{
    ...
    if (obj->otyp == ICE_BOX || Is_box(obj) || obj->otyp == BOULDER
        ...
        You("不能把%s装进%s.", buf, the(xname(current_container)));
        return 0;
    }
...
struct obj *item;
{
    ...
    if (item->dknown)
        ...
        You("%s %s 消失了!", Blind ? "注意到" : "看见", doname(item));
...
{
    static NEARDATA const char sc[] = "薛定谔的猫";
    struct obj *deadcat;
...
boolean more_containers; /* True iff #loot multiple and this isn't last one */
{
    ...
    if (!outokay) /* preformat the empty-container message */
        Sprintf(emptymsg, "%s %s是空的.", Ysimple_name2(current_container),
    ...
    if (loot_in) {
        ...
        if ((otmp = getobj(stashable, "藏匿")) != 0) {  //stash
...
boolean outokay, inokay, alreadyused, more_containers;
{
    ...
    any.a_int = 1; /* ':' */
    Sprintf(buf, "查看%s的里面", simpleonames(obj));
    ...
    if (inokay) {
        ...
        add_menu(win, NO_GLYPH, &any, menuselector[any.a_int], 0, ATR_NONE,
                 buf, MENU_UNSELECTED);
        any.a_int = 6; /* 's' */
        Sprintf(buf, "把一项藏匿到%s", simpleonames(obj));
        add_menu(win, NO_GLYPH, &any, menuselector[any.a_int], 0, ATR_NONE,
                 buf, MENU_UNSELECTED);
    }
    any.a_int = 0;
        ...
        for (otmp = box->cobj; otmp; otmp = nobj) {
            ...
            if (highdrop) {
                ...
                if (altarizing) {
                    ...
                    pline("%s %s到%s上.", Doname2(otmp),
                          otense(otmp, "掉落"), surface(ox, oy));
                } else {
