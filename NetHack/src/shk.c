...
STATIC_VAR const char and_its_contents[] = "和它里面的东西";
STATIC_VAR const char the_contents_of[] = "里的东西";
...
struct monst *shkp;
{
    ...
    eshkp->robbed += total;
    You("偷走了价值%ld %s的商品.", total, currency(total));
    ...
    for (pass = this_shkp ? 0 : 1; pass <= 1; pass++)
        for (shkp = next_shkp(fmon, FALSE); shkp;
            ...
            if ((amt = eshkp->credit) != 0)
                You("有%ld %s信用在%s %s.", amt, currency(amt),
                    s_suffix(shkname(shkp)),
...
boolean ininv, dummy, silent;
{
    ...
    if (!Deaf && !muteshk(shkp) && !silent) {
        ...
        if (!ltmp) {
            pline("%s对%s没兴趣.", Shknam(shkp), the(xname(obj)));
            return;
        }
        if (!ininv) {
            pline("%s%s会花费你%ld %s.", (obj->quan > 1L) ? "每个" : "",
                  The(xname(obj)), ltmp,
                  currency(ltmp));
        } else {
    ...
    if (isgold || cgold) {
        ...
        if (eshkp->debit >= gltmp) {
            ...
            if (eshkp->credit == delta)
                You("建立了%ld %s的信用.", delta,
                    currency(delta));
            else
                pline("%ld %s增加到了你的信用; 现在总共%ld %s.",
...
const char *Izchak_speaks[] = {
    ...
    "%s说: '慢下来.  想清楚.'",
    "%s说: '你做事要一件件做.'",
    "%s说: '我不喜欢普菲咖啡... 给我一杯哥伦比亚咖啡.'",
    "%s在任何事情上获得开发团队的同意都是非常困难的.",
    "%s说他注意到那些侍奉他们神的人将会顺利.",
    "%s说: '不要试图偷我的东西 -  我有地位高的朋友!'",
    "%s说: '将来你可能很需要这个商店的什么东西.'",
    "%s评论死亡山谷就像是一个关口."
};
...
struct obj *obj_absorber, *obj_absorbed;
{
    ...
    if (!bp_absorber && !bp && !obj_absorber->no_charge) {
        ...
        if (saleable(shkp, obj_absorbed)) {
            if (eshkp->debit >= amount) {
                ...
                if (eshkp->credit == delta)
                    pline_The("%s建立了%ld %s的信用.",
                              obj_typename(obj_absorbed->otyp),
                              delta, currency(delta));
                else
                    pline_The("%s增加了%ld %s%s%ld %s.",
                              obj_typename(obj_absorbed->otyp),
                              delta, currency(delta),
                              "信用; 现在总共有",
                              eshkp->credit, currency(eshkp->credit));
    ...
    if (bp && (obj_absorber->no_charge
        ...
        verbalize("你欠我%ld %s因为你%s把我的%s和你的%s",
                  amount, currency(amount),
                  ANGRY(shkp) ? "居然敢" : "",
                  obj_typename(obj_absorbed->otyp),
                  ANGRY(shkp) ? "那批臭货混在一起!" : "混在一起了.");
        return;
    }
    /**************************************************************
