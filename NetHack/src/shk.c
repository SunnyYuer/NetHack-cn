...
char *enterstring;
{
    ...
    if (Invis) {
        pline("%s 感觉到了你的存在.", shkname(shkp));
        if (!Deaf && !muteshk(shkp))
            verbalize("不欢迎隐形的顾客!");
        else
            pline("%s 站直挺立就好像%s知道你在那里.",
    ...
    if (ANGRY(shkp)) {
        ...
        if (!Deaf)
            pline("%s 喃喃地咒骂商店扒手.", shkname(shkp));
        else
            pline("%s 正在梳理%s库存清单.",
                  Shknam(shkp),
                  mhis(shkp));
...
register boolean silentkops;
{
    ...
    if (!inhishop(shkp)) {
        ...
        if (vanished)
            pline("满意的, %s 突然消失了!", shk_nam);
    } else if (wasmad)
        pline("%s 平静下来.", Shknam(shkp));
...
proceed:
    ...
    if (!eshkp->billct && !eshkp->debit) {
        ...
        if (!ltmp && NOTANGRY(shkp)) {
            ...
            pline("但自从%s 商店在最近被抢劫了,", mhis(shkp));
            pline("你%s补偿%s %s损失.",
            ...
            You("试图让%s 息怒通过给%s1000 金币.",
                x_monnam(shkp, ARTICLE_THE, "生气的", 0, FALSE), mhim(shkp));
            pay(1000L, shkp);
    ...
    if (!ANGRY(shkp) && paid) {
        if (!Deaf && !muteshk(shkp))
            verbalize("感谢在%s %s购物!", s_suffix(shkname(shkp)),
                  shtypes[eshkp->shoptype - SHOPBASE].name);
        else
            pline("%s 对于你在%s%s 购物感激地点头!",
                    Shknam(shkp),
                    mhis(shkp),
                    shtypes[eshkp->shoptype - SHOPBASE].name);
    }
...
int croaked;
{
    ...
    if (numsk > 1) {
        if (cansee(shkp->mx, shkp->my) && croaked) {
            ...
            if (has_head(shkp->data) && !rn2(2))
                Sprintf(takes, ", 摇摇%s %s,", mhis(shkp),
                        mbodypart(shkp, HEAD));
    ...
    if (eshkp->following || ANGRY(shkp) || take) {
        ...
        if (loss > umoney || !loss || roomno == eshkp->shoproom) {
            ...
            context.botl = 1;
            pline("%s %s 你的所有财产.", shkname(shkp), takes);
            taken = TRUE;
            ...
            money2mon(shkp, loss);
            context.botl = 1;
            pline("%s %s %ld %s %s欠%s.", Shknam(shkp),
                  takes, loss, currency(loss),
                  strncmp(eshkp->customer, plname, PL_NSIZ) ? "" : "你",
                  mhim(shkp));
...
char *buf;
{
    ...
    if (is_vampire(youmonst.data))
        ...
        Strcat(buf, !is_human(youmonst.data) ? "生物" : (flags.female)
                                                                 ? "女士"
                                                                 : "先生");
}
...
boolean peaceful, silent;
{
    ...
    if (peaceful) {
        ...
        if (!silent) {
            ...
            if (credit_use) {
                if (ESHK(shkp)->credit) {
                    You("有 %ld %s 信用剩余.", ESHK(shkp)->credit,
            ...
            if (obj->oclass == COIN_CLASS)
                You("%s欠%s %ld %s!", still, shkname(shkp), value,
                    currency(value));
            else
                You("%s欠%s %ld %s 对%s!", still, shkname(shkp),
                    value, currency(value), (obj->quan > 1L) ? "它们" : "它");
        }
    } else {
        ESHK(shkp)->robbed += value;
  ...
  "谢谢你对这个最近被掠夺的商店进行补充的贡献.");
    ...
    if (!shkmoney) {
        ...
        if (sell_how == SELL_NORMAL || auto_credit) {
            ...
            pline("%s 现在不能付给你.", shkname(shkp));
            Sprintf(qbuf, "你能接受 %ld %s 的信用对于", tmpcr,
                    currency(tmpcr));
        ...
        if (!sell_response) {
             ...
             */
            Sprintf(qbuf, "%s %s出价 %ld 金币对于%s%s ",
                    shkname(shkp), short_funds ? "仅" : "", offer,
                    (cltmp && !ltmp)
                        ? ((yourc == 1L) ? "你的物品在" : "你的物品在")
                        : "",
                    obj->unpaid ? "" : "你的");
    ...
    if (!fall) {
        ...
        if (distu(shkp->mx, shkp->my) > 2) {
            ...
            if (distu(shkp->mx, shkp->my) > 2) {
                ...
                pline("%s %s, 并%s你的背包!", shkname(shkp),
                      makeplural(locomotion(shkp->data, "跳跃")), grabs);
        } else
            pline("%s %s你的背包!", shkname(shkp), grabs);
