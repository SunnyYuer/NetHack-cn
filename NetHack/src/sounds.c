...
register struct monst *mtmp;
{
    ...
    if (Hallucination)
        ...
        switch (mtmp->data->msound) {
        case MS_MEW:
            yelp_verb = (!Deaf) ? "吼叫" : "弓背";
            break;
        case MS_BARK:
        case MS_GROWL:
            yelp_verb = (!Deaf) ? "嗷叫" : "退缩";
            break;
        case MS_ROAR:
            yelp_verb = (!Deaf) ? "咆哮" : "吓唬";
            break;
        case MS_SQEEK:
            yelp_verb = (!Deaf) ? "尖叫" : "抖动";
            break;
        case MS_SQAWK:
            yelp_verb = (!Deaf) ? "嘎嘎作响" : "摆动";
            break;
        case MS_WAIL:
            yelp_verb = (!Deaf) ? "哀号" : "畏缩";
            break;
        }
    if (yelp_verb) {
...
register struct monst *mtmp;
{
    ...
    case MS_HUMANOID:
        ...
        /* Specific monsters' interests */
            ...
            verbl_msg = (gnomeplan == 1) ? "第一阶段, 收集内裤."
                                         : "第三阶段, 利润!";
        else
            switch (monsndx(ptr)) {
            case PM_HOBBIT:
